...
<div class="space-y-8">
    ...
    <form method="POST" action="{{ route('profile.update') }}">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between mb-8">
            ...
            <button type="submit" class="inline-flex items-center rounded-2xl bg-[var(--color-primary)] px-4 py-2 text-sm font-semibold text-[var(--color-primary-foreground)] transition hover:opacity-90">Simpan Perubahan</button>
        </div>
                    ...
                    <div class="mt-6 grid gap-4 sm:grid-cols-2">
                        <label class="space-y-2 text-sm">
                            ...
                            <input type="text" name="name" value="{{ old('name', $user->name) }}"
                                class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:border-[var(--color-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]/10" required />
                        </label>
                        <label class="space-y-2 text-sm">
                        ...
                        <label class="space-y-2 text-sm">
                            ...
                            <input type="text" name="phone" value="{{ old('phone', $profile?->phone ?? '') }}"
                                class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:border-[var(--color-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]/10" />
                        </label>
                        <label class="space-y-2 text-sm">
                        ...
                        <label class="space-y-2 text-sm sm:col-span-2">
                            ...
                            <textarea name="bio" rows="3"
                                class="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 focus:border-[var(--color-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-primary)]/10 resize-none">{{ old('bio', $profile?->bio ?? '') }}</textarea>
                        </label>
                    </div>
                </div>
