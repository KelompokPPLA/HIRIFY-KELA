...
<div class="max-w-4xl mx-auto">
    ...
    <div class="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-[0_8px_30px_rgb(0,0,0,0.02)]">
        ...
        @if($session->material_file)
            ...
            @endphp
            <div class="p-8 sm:p-10 border-b border-slate-100 bg-slate-50/10">
                <h3 class="text-lg font-extrabold text-slate-900 tracking-tight mb-5 flex items-center gap-2">
                    ...
                    Pratonton Materi Sesi
                </h3>

                @if($isVideo)
                    <div class="rounded-2xl border border-slate-200 overflow-hidden bg-slate-950 shadow-md">
                        <video class="w-full h-auto aspect-video max-h-[480px] mx-auto block" src="{{ $fileUrl }}" controls playsinline></video>
                    </div>
                @elseif($isPdf)
                    <div class="space-y-6">
                        <div class="rounded-2xl border border-slate-200 overflow-hidden bg-white p-6 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-sm">
                            <div class="flex items-center gap-4">
                                <div class="w-14 h-14 rounded-2xl bg-rose-50 border border-rose-100 flex items-center justify-center text-rose-500 shrink-0 shadow-sm">
                                    <svg class="w-8 h-8" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 00-3.375-3.375h-1.5A1.125 1.125 0 0113.5 7.125v-1.5a3.375 3.375 0 00-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 00-9-9z"/>
                                    </svg>
                                </div>
                                <div class="min-w-0">
                                    <h4 class="font-extrabold text-slate-800 text-sm truncate max-w-md">{{ basename($session->material_file) }}</h4>
                                    <p class="text-xs text-slate-400 font-semibold mt-1">Dokumen Materi PDF Sesi</p>
                                </div>
                            </div>
                            <a href="{{ $fileUrl }}" target="_blank"
                                class="inline-flex items-center gap-2 px-6 py-3 bg-[#00bee4] text-white rounded-full text-xs font-extrabold uppercase tracking-wider hover:bg-[#00a3c4] transition duration-200 shadow-md shadow-cyan-500/15 shrink-0 self-stretch sm:self-auto text-center justify-center">
                                Buka di Tab Baru
                            </a>
                        </div>
                        
                        <!-- PDF Embedded Preview -->
                        <div class="rounded-2xl border border-slate-200 overflow-hidden bg-slate-100 shadow-inner">
                            <iframe src="{{ $fileUrl }}#toolbar=0" class="w-full h-[600px] border-0" allow="autoplay"></iframe>
                        </div>
                    </div>
                @else
                    <div class="rounded-2xl border border-slate-200 overflow-hidden bg-white p-6 flex flex-col sm:flex-row items-center justify-between gap-6 shadow-sm">
                        <div class="flex items-center gap-4">
                            <div class="w-14 h-14 rounded-2xl bg-slate-50 border border-slate-100 flex items-center justify-center text-slate-500 shrink-0 shadow-sm">
                                <svg class="w-8 h-8" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                            ...
                            <div class="min-w-0">
                                <h4 class="font-extrabold text-slate-800 text-sm truncate max-w-md">{{ basename($session->material_file) }}</h4>
                                <p class="text-xs text-slate-400 font-semibold mt-1">Berkas Materi Sesi</p>
                            </div>
                        </div>
                        <a href="{{ $fileUrl }}" target="_blank"
                            class="inline-flex items-center gap-2 px-6 py-3 bg-slate-900 text-white rounded-full text-xs font-extrabold uppercase tracking-wider hover:bg-slate-800 transition duration-200 shadow-md shrink-0 self-stretch sm:self-auto text-center justify-center">
                            Unduh Berkas
                        </a>
                    </div>
                @endif
